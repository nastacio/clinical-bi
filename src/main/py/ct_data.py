...
def load_data(y_name='status', db_props='aact.properties'):
    ...
    "    d.allocation, d.intervention_model, d.primary_purpose, i2.drug_recency, " + \
    ...
    "    sponsors as sp, sponsors as sp2, design_group_interventions as dgi, designs as d, " + \
    ...
    "WHERE s.nct_id=cv.nct_id AND s.nct_id=sp.nct_id AND s.nct_id=i.nct_id AND s.nct_id=sp2.nct_id AND s.nct_id=e.nct_id  AND s.nct_id=dgi.nct_id AND s.nct_id=d.nct_id AND s.nct_id=i2.nct_id " + \
...
#    print(df.groupby('phase').count())
    ...
    df.loc[df.allocation == 'Non-Randomized', 'allocation_type'] = 1
    df.loc[df.allocation == 'Randomized', 'allocation_type'] = 2
    ...
    df.drop(columns=['start_date','overall_status','sponsor_type', 'gender', 'phase', 'study_type', 
                     'has_us_facility', 'allocation', 'intervention_model', 'primary_purpose', 'drug_recency'], inplace=True)
